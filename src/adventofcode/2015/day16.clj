...
(defn parse-tape [s]
  (->> (str/split s #", |\n" )
       (map (fn [line]
              (let [[compound distinct-kinds] (str/split line #": ")]
                [(keyword compound)
                 (parse-long distinct-kinds)])))
       (into (array-map))))
...
(defn parse-message [m]
  (let [[ _ id tape] (re-matches #"Sue (\d+): (.*)" m)]
    [ (parse-long id) (parse-tape tape)]))


(def remembered (parse-tape example))

(defn ok? [items]
  (every?
   (fn [ id]
     (== (get remembered id) (get items id))
     )
   (set/intersection (set (keys remembered)) (set (keys items)))))

(defn ok=2? [items]
  (every?
   (fn [ id]
     (case id
       (:cats :trees) (< (get remembered id) (items id))
       (:pomeranians :goldfish) (> (get remembered id) (items id))
       (== (get remembered id) (get items id)))
     )
   (set/intersection (set (keys remembered)) (set (keys items)))))
